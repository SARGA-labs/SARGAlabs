...
export async function getDriveFiles(folderId: string) {
  try {
    // Graceful handling for placeholder
    if (folderId === 'sMdC6-NTOezE657qk0uH2WBw_7Z49hBD') return [];
    ...
    // Log error but return empty array to keep UI valid
    console.error('Error fetching drive files (likely permission or ID issue):', error.message);
    return [];
  }
}
